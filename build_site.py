...
import requests
import pandas as pd
import numpy as np # Import numpy for NaN checking/replacement
    ...
    try:
        ...
        for product_name, price_info in market_data.items():
            if isinstance(price_info, dict) and 'ask' in price_info and 'bid' in price_info:
                 ...
                 if vendor_price == -1 or vendor_price is None: vendor_prices[product_name] = None # Store as None
                 else:
...
def calculate_trends(df, products):
    ...
    # Convert NaN/NA trends to None for JSON compatibility before returning
...
def main():
    ...
    # 1. Market Summary (List of products with latest info)
    market_summary = []
    if not combined_df.empty:
        latest_data_map = combined_df.groupby('product').last() # Relies on data being sorted by time
        for product in all_products:
            if product in latest_data_map.index:
                ...
                market_summary.append({
                    ...
                    'trend': product_trends.get(product) # Already None if missing
                })
    summary_path = os.path.join(OUTPUT_DATA_DIR, 'market_summary.json')
    try:
        with open(summary_path, 'w') as f:
            json.dump(market_summary, f, allow_nan=False) # Ensure allow_nan is False
        logging.info(f"Saved market summary to {summary_path}")
    except ValueError as ve: logging.error(f"ValueError saving market summary JSON (likely NaN present): {ve}")
    ...
    if not combined_df.empty:
        ...
        # *** FIX: Explicitly replace NaN/NA with None before creating dict list ***
        # Replace in 'buy' and 'ask' columns specifically
        chart_history_data['buy'] = chart_history_data['buy'].replace({np.nan: None})
        chart_history_data['ask'] = chart_history_data['ask'].replace({np.nan: None})
        # Also handle pd.NA if it exists (might depend on pandas version)
        chart_history_data['buy'] = chart_history_data['buy'].replace({pd.NA: None})
        chart_history_data['ask'] = chart_history_data['ask'].replace({pd.NA: None})

        # Now convert to dictionary list
        ...
        try:
            with open(history_path, 'w') as f:
                # Use allow_nan=False to catch any remaining issues
                json.dump(chart_history_list, f, allow_nan=False)
            logging.info(f"Saved full market history to {history_path}")
        except ValueError as ve:
            logging.error(f"ValueError saving market history JSON (likely NaN present despite replacement): {ve}")
            # Log a sample of the list that failed if possible
            logging.error(f"Sample data causing error: {str(chart_history_list[:5])}")
        except Exception as e:
    ...
    try:
        ...
        html_context = { 'last_update': datetime.now().strftime('%Y-%m-%d %H:%M:%S') }
